...
class Polygon(Obstacle):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if absolute_edge_position:
            # change edge points to be wrt to center point
        ...
        if self.dim == 2:
            ...
            self.ind_tiles = indices_of_tiles
            self.n_planes = indices_of_tiles.shape[0]
    ...
    def update_margin(self):
        if self._margin_absolut > 0:
            if self.dim == 2:
                ...
                tangent_dirs = np.zeros((self.dim, 2))
                tangent1 = self.edge_points[:, 0] - self.edge_points[:, -1]
                tangent1 = tangent1 / np.linalg.norm(tangent1)

                for ii in range(self.n_planes):
                    tangent0 = tangent1
                    tangent1 = self.edge_points[:, (ii + 1) % self.n_planes] - self.edge_points[:, ii]
                    tangent1 = tangent1 / np.linalg.norm(tangent1)
                    ...
                    if (not self.is_boundary and np.cross(tangent0, tangent1) > 0) or (
                        ...
                        self.edge_margin_points[:, ii, 0] = (
                            self.edge_points[:, ii] + self.normal_vector[:, ii - 1] * self._margin_absolut * norm_factor
                        )
                        self.edge_margin_points[:, ii, 1] = (
                            self.edge_points[:, ii] + self.normal_vector[:, ii] * self._margin_absolut * norm_factor
                        )
                    else:
                        angle_corner = np.arccos(tangent0.T.dot(-tangent1))

                        dir_mean = self.normal_vector[:, ii - 1] + self.normal_vector[:, ii]
                        dir_mean = dir_mean / np.linalg.norm(dir_mean) * norm_factor

                        margin_hat = self.margin_absolut / np.sin(angle_corner / 2.0)

                        self.edge_margin_points[:, ii, 0] = self.edge_margin_points[:, ii, 1] = (
    ...
    def get_maximal_distance(self):
        dist_edges = np.linalg.norm(
            self.edge_points - np.tile(self.center_position, (self.edge_points.shape[1], 1)).T, axis=0
        )
    ...
    def calculate_normalVectorAndDistance(self, edge_points=None):
        ...
        if self.dim == 2:
            if len(edge_points.shape) == 2:
                ...
                for ii in range(self.n_planes):
                    normal_vector[:, ii] = edge_points[:, (ii + 1) % normal_vector.shape[1], 0] - edge_points[:, ii, 1]
            else:
                raise ValueError("")
            ...
            for ii in range(self.n_planes):
                tangent_0 = self.edge_points[:, self.ind_tiles[ii, 1]] - self.edge_points[:, self.ind_tiles[ii, 0]]

                tangent_1 = self.edge_points[:, self.ind_tiles[ii, 2]] - self.edge_points[:, self.ind_tiles[ii, 1]]
        ...
        if len(edge_points.shape) == 2:
            ...
            for ii in range(self.n_planes):
                normalDistance2center[ii] = normal_vector[:, ii].T.dot(edge_points[:, ii, 0])
    ...
    def draw_obstacle(
        ...
        # self._boundary_points = np.hstack((self.edge_points, np.reshape(self.edge_points[:,0],(self.dim,1))))

        if self.margin_absolut:
            self._boundary_points_margin = np.zeros((self.dim, 0))
            for ii in range(self.n_planes):
                if not is_one_point(self.edge_reference_points[:, ii, 0], self.edge_reference_points[:, ii, 1]):
                    ...
                    v1 = self.edge_reference_points[:, ii, 0] - self.edge_points[:, it_edge]
                    v1 = v1 / np.linalg.norm(v1)

                    v2 = self.edge_reference_points[:, ii, 1] - self.edge_points[:, it_edge]
                    ...
                    angles = np.arange(
                        angle1, angle1 + angle_difference_directional(angle2, angle1), point_density * direction
                    )

                    self._boundary_points_margin = np.hstack(
                        (
                            self._boundary_points_margin,
                            self.margin_absolut * np.vstack((np.cos(angles), np.sin(angles)))
                            + np.tile(self.edge_points[:, it_edge], (angles.shape[0], 1)).T,
                        )
                    )
                self._boundary_points_margin = np.hstack(
                    (self._boundary_points_margin, self.edge_reference_points[:, ii, 1].reshape(self.dim, 1))
                )
        ...
        if add_circular_margin:
            ...
            obs_margin_circle = self.margin_absolut * np.vstack((np.cos(angles), np.sin(angles)))
            ...
            for ii in range(num_edges):
                self._boundary_points_with_margin = np.hstack(
                    (
                        ...
                        + obs_margin_circle[:, ii * n_curve_points : (ii + 1) * n_curve_points + 1],
                    )
                )
            self._boundary_points_with_margin = np.hstack(
                (self._boundary_points_with_margin, self._boundary_points_with_margin[:, 0].reshape(2, 1))
            )
        else:
    ...
    def get_distances_and_normal_to_surfacePannels(self, position, edge_points=None, in_global_frame=False):
        ...
        for ii in range(edge_points.shape[1]):
            ...
            normal_vectors[:, ii] = normal_vectors[:, ii] / np.linalg.norm(normal_vectors[:, ii], axis=0)
    ...
    def get_distance_to_hullEdge(self, position, in_global_frame=False):
        ...
        if self.dim == 2:
            ...
            if not self.margin_absolut:
                for jj in np.arange(n_points)[~zero_mag]:
                    ...
                    if magnitude_angles[ind_low] == 0:
                        ...
                        dist2hull[jj], dist_tangent = np.linalg.lstsq(
                            np.vstack((position_dir[:, jj], -surface_dir)).T, self.edge_points[:, ind_low], rcond=-1
                        )[0]
                ...
                for jj in np.arange(n_points)[~zero_mag]:
                    angle_to_reference = get_angle_space(
                        null_direction=position_dir[:, jj], directions=self.edge_reference_points[:, :, 0]
                    )
                    ...
                    if is_one_point(
                        self.edge_reference_points[:, ind_low, 0], self.edge_reference_points[:, ind_low, 1]
                    ):  # one point
                        surface_dir = (
                            self.edge_reference_points[:, ind_high, 0] - self.edge_reference_points[:, ind_low, 0]
                        )
                        edge_point = self.edge_reference_points[:, ind_low, 0]

                    else:
                        angle_hull_double_low = get_angle_space(
                            null_direction=position_dir[:, jj], directions=self.edge_reference_points[:, ind_low, 1]
                        )
                        ...
                        if angle_hull_double_low > 0:
                            ...
                            CC = pp[0] * pp[0] + pp[1] * pp[1] - self.margin_absolut * self.margin_absolut
                            ...
                            surface_dir = (
                                self.edge_reference_points[:, ind_high, 0] - self.edge_reference_points[:, ind_low, 1]
                            )
                            edge_point = self.edge_reference_points[:, ind_low, 1]

                    # Get distance to hull for both ifs
                    dist2hull[jj], dist_tangent = np.linalg.lstsq(
                        np.vstack((position_dir[:, jj], -surface_dir)).T, edge_point, rcond=-1
                    )[0]
    ...
    def adapt_normal_to_arc_extension(self, position, normal_vector, in_global_frame=False):
        ...
        for ii in range(self.edge_reference_points.shape[1]):
            if not is_one_point(self.edge_reference_points[:, ii, 0], self.edge_reference_points[:, ii, 1]):
                ...
                angle_low = np.arctan2(self.edge_reference_points[1, ii, 0], self.edge_reference_points[0, ii, 0])
                angle_high = np.arctan2(self.edge_reference_points[1, ii, 1], self.edge_reference_points[0, ii, 1])
                ...
                if angle_is_in_between(angle_position, angle_low, angle_high):
                    ...
                    break
        if in_global_frame:
            normal_vector = self.transform_relative2global(normal_vector)
    ...
    def get_normal_direction(self, position, in_global_frame=False, normalize=True, normal_calulation_type="distance"):
            ...
            if sys.version_info > (3, 0):  # TODO: remove in future
                ...
                for jj in range(temp_edge_points.shape[1]):
                    ...
                    for kk in index_unique:
                        if is_one_point(temp_edge_points[:, jj], temp_edge_points[:, kk]):
                            is_unique = False
                            break
                    if is_unique:
        ...
        distances2plane, normal_vectors = self.get_distances_and_normal_to_surfacePannels(
        ...
        if (not self.is_boundary) and mag_ref_point and self.get_gamma(reference_point_temp) > 1:
            ...
            if self.margin_absolut:
                for pp in range(self.n_planes):
                    ...
                    outer_edge_point = self.edge_points[:, pp] + self.margin_absolut * (n0 + n1)
                    ...
                    if np.cross(vec_ref, t0) > 0:
                        if np.cross(vec_ref, t1) > 0:
                            ...
                            self.edge_reference_points[:, pp, 0] = self.edge_reference_points[
                                :, pp, 1
                            ...
                            self.edge_reference_points = np.hstack(
                                (
                                    np.reshape(self.edge_margin_points[:, :pp, :], (self.dim, -1, 2)),
                                    np.reshape(np.tile(reference_point_temp, (2, 1)).T, (self.dim, 1, 2)),
                                    np.reshape(self.edge_margin_points[:, pp:, :], (self.dim, -1, 2)),
                                )
                            )
                            self.n_planes += 1
                        ...
                        self.edge_reference_points[:, qq, 0] = (
                            tang_points[:, 0] if np.cross(tt[:, 0], tt[:, 1]) > 0 else tang_points[:, 1]
                        )
                        ...
                        if np.cross(vec_ref, t0) > 0 and np.cross(vec_ref, t1) > 0:
                            self.edge_reference_points = np.hstack(
                                (
                                    np.reshape(self.edge_margin_points[:, : (pp + 1), :], (self.dim, -1, 2)),
                                    np.reshape(np.tile(reference_point_temp, (2, 1)).T, (self.dim, 1, 2)),
                                    np.reshape(self.edge_margin_points[:, pp:, :], (self.dim, -1, 2)),
                                )
                            )
                            ...
                            if np.cross(tt[:, 0], tt[:, 1]) > 0:
                                ...
                                self.edge_reference_points[:, (pp), 1] = tang_points[:, 1]
                                self.edge_reference_points[:, (pp + 2) % self.n_planes, 0] = tang_points[:, 0]
                            else:
