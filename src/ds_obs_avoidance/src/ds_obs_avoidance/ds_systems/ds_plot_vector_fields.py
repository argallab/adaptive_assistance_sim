...
def plot_dynamical_system(
    DynamicalSystem=None, x_lim=None, y_lim=None, n_resolution=15, figsize=(10, 7), plottype="quiver", axes_equal=True
):
    """ Evaluate the dynamics of the dynamical system. """
    assert DynamicalSystem is not None
    dim = 2 #todo get dim from DynamicalSystem
    if x_lim is None:
        x_lim = [-10, 10]
    ...
    nx = n_resolution
    ny = n_resolution
    x_vals, y_vals = np.meshgrid(np.linspace(x_lim[0], x_lim[1], nx), np.linspace(y_lim[0], y_lim[1], ny))
    ...
    if plottype == "quiver":
        plt.quiver(positions[0, :, :], positions[1, :, :], velocities[0, :, :], velocities[1, :, :], color="blue")
    elif plottype == "streamplot":
        ax.streamplot(x_vals, y_vals, velocities[0, :].reshape(nx, ny), velocities[1, :].reshape(nx, ny), color="blue")
    else:
