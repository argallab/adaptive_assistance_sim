...
from ds_obs_avoidance.ds_containers.obstacle_container import ObstacleContainer
...
from ds_obs_avoidance.ds_utils.linalg import *
...
class GradientContainer(ObstacleContainer):
    ...
    def append(self, value):
        ...
        if True:
            ...
            distance_matrix = np.hstack(
                ((-1) * np.ones((len(self), 1)), np.vstack((distance_matrix, (-1) * np.ones((1, len(self) - 1)))))
            )
    ...
    def __delitem__(self, key):  # Compatibility with normal list.
        ...
        if len(self) == 0:
            ...
            self._boundary_reference_points = np.delete(self._boundary_reference_points, (key), axis=1)
            self._boundary_reference_points = np.delete(self._boundary_reference_points, (key), axis=2)
        ...
        for ii in range(len(self)):
            ...
            if np.sum(weights):
                ...
                for jj in range(len(self)):
                    ...
                    reference_point = reference_point + self[ii].transform_global2relative(ref) * weights[jj]
    ...
    def update_boundary_reference_points(
        ...
        for ii in range(len(self) - 1):
            ...
            for jj in range(ii + 1, len(self)):
                ...
                dist_ii2jj = np.linalg.norm(self[ii].center_position - self[jj].center_position)

                # Don't calculate if obstacles are too far away from each other
                if dist_ii2jj - (size_ii + size_jj) > max(size_ii, size_jj) * mult_consideration_dist:
                    # print("Too far away")
                    continue

                # TODO no circular obstacles.
                # # Speed up process for circular obstacles
                # if ((isinstance(self[ii], CircularObstacle)
                #     and isinstance(self[jj], CircularObstacle))
                #     or need_for_speed):

                #     dist, ref_point1, ref_point2 = self.get_boundary_reference_point_simplified(
                #         self[ii], self[jj]
                #     )
                ...
                margin = 1e-4
                if (
                    self[ii].get_gamma(surf_points[:, 1], in_global_frame=True) <= 1 + margin
    ...
    def angle_gradient_descent(
        self, obs0, obs1, angles, NullMatrices, contact_err=1e-2, convergence_err=1e-3, max_it=100
    ):
        ...
        for obs, ii in zip([obs0, obs1], [0, 1]):
            ...
            surface_points[:, ii] = obs.get_local_radius_point(direction=direction_angle, in_global_frame=True)
        ...
        while not is_intersecting:
            ...
            delta_angle = (
                0.5
                / dist_magnitude
                * np.array([-dist_dir.dot(surface_derivatives[:, 0]), dist_dir.dot(surface_derivatives[:, 1])])
            )
        ...
        for obs, ii in zip((obs0, obs1), [0, 1]):
            ...
            reference_points[:, ii] = obs.get_local_radius_point(direction=dir0, in_global_frame=True)
        ...
        if is_intersecting:
            reference_point = self.gamma_gradient_descent(obs0, obs1, common_point=np.mean(reference_points, axis=1))
            dist = 0
    ...
    def derivative_gamma_sum(self, position, obs0, obs1, grad_pow=4, delta_dist=1e-6, gamma_type="proportional"):
        ...
        for dd in range(dim):
            ...
            delta_gamma = self.get_gamma_cost_function(gamma_high) - self.get_gamma_cost_function(gamma_low)
            ...
            delta_gamma += self.get_gamma_cost_function(gamma_high, obs1.is_boundary) - self.get_gamma_cost_function(
                gamma_low, obs1.is_boundary
            )
