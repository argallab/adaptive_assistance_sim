...
from ds_obs_avoidance.ds_containers.container import BaseContainer
...
class ObstacleContainer(BaseContainer):
    ...
    def reset_intersections(self, index=None):
        if index is None:
            self._distance_matrix = Intersection_matrix(len(self._obstacle_list), self.dimension)
        else:
        ...
        if True:
            ...
            if np.sum(ind_singles):  # nonzero
                self._family_label[ind_singles] = np.arange(
                    len(intersecting_obs), len(intersecting_obs) + np.sum(ind_singles)
                )
    ...
    def get_relative_angle_to_family(self, ind_newObstacle, position):
        ...
        angle_space_difference += get_angle_space(ref_dir, transform_direction)

        for ii in range(0, len(short_connection) - 1):
            basis_direction = transform_direction
            transform_direction = (
                self.intersection_points[ind_short_connection[ii + 1], ind_short_connection[ii]]
                - self[ind_short_connection[ii]].global_reference_point
            )

            angle_space_difference += get_angle_space(basis_direction, transform_direction)
