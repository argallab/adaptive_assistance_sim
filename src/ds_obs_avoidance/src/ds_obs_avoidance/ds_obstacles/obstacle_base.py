...
class Obstacle(object):
    ...
    def __init__(
        ...
        if sum(np.abs(self.linear_velocity)) or np.sum(self.angular_velocity) or self.timeVariant:
    ...
    def compute_rotation_matrix(self):
        ...
        if self.dim == 2:
            self.rotMatrix = np.array([[cos(orientation), -sin(orientation)], [sin(orientation), cos(orientation)]])
    ...
    def update_position_and_orientation(
        ...
        if self.dim == 2:
            ...
            self.linear_velocity = (
                k_linear_velocity * self.linear_velocity + (1 - k_linear_velocity) * new_linear_velocity
            )
            self.center_position = k_position * (self.linear_velocity * dt + self.center_position) + (
                1 - k_position
            ) * (position)

            # Periodic Weighted Average
            self.angular_velocity = (
                k_angular_velocity * self.angular_velocity + (1 - k_angular_velocity) * new_angular_velocity
            )
    ...
    def are_lines_intersecting(direction_line, passive_line):
        ...
        if LA.det(connection_matrix):  # nonzero value
            direction_factors = LA.inv(connection_matrix).dot(
                np.array(passive_line["point_start"]) - np.array(direction_line["point_start"])
            )

            # Smooth because it's a tangent
            if direction_factors[0] >= 0:
                if direction_factors[1] >= 0 and LA.norm(direction_factors[1] * connection_passive) <= LA.norm(
                    connection_passive
                ):
    ...
    def draw_reference_hull(self, normal_vector, position):
        ...
        ref_dir = self.transform_relative2global_dir(
            self.get_reference_direction(position, in_global_frame=False, normalize=True)
        )
