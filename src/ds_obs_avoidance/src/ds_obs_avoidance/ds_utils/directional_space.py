...
def get_angle_space(direction, null_direction=None, null_matrix=None, normalize=None, OrthogonalBasisMatrix=None):
    """ Get the direction transformed to the angle space with respect to the 'null' direction."""

    ## Given a NULL DIRECTION (2,) and DIRECTION (2,)
    ## computes the angle from null_direction to direction
    """
    direction: (2, ) or (3, ) direction vector
    """
    ...
    return direction_directionSpace  # for 3d gives theta, phi
...
def get_angle_space_inverse(dir_angle_space, null_direction=None, null_matrix=None, NullMatrix=None):
    ...
    if norm_directionSpace:
        directions = null_matrix.dot(
            np.hstack(
                (np.cos(norm_directionSpace), np.sin(norm_directionSpace) * dir_angle_space / norm_directionSpace)
            )
        )
    else:
...
def get_directional_weighted_sum(
    null_direction, directions, weights, total_weight=1, normalize=True, normalize_reference=True
):
    ...
    directions_directionSpace[:, ind_nonzero] = directions_directionSpace[:, ind_nonzero] / np.tile(
        norm_dirSpace[ind_nonzero], (dim - 1, 1)
    )
...
def get_angle_space_array_old(directions, null_direction, null_matrix=None, normalize=True, OrthogonalBasisMatrix=None):
    ...
    if np.sum(cos_directions > 1) or np.sum(cos_directions < -1):
        ...
        warnings.warn("Cosines value out of bound.")
