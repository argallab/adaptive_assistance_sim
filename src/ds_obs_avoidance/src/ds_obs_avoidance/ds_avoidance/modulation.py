...
def obs_avoidance_interpolation_moving(
    ...
    """
    if x is not None:
        ("Depreciated, don't use x as position argument.")
        position = x
    else:
        x = position
    ...
    N_obs = len(obs)
    if not N_obs:  # No obstacle, therefore nothing to modulate
    ...
    for n in range(N_obs):
        Gamma_proportional[n] = obs[n].get_gamma(
            pos_relative[:, n], in_global_frame=evaluate_in_global_frame, gamma_distance=gamma_distance,
        )
    ...
    for n in np.arange(N_obs)[ind_obs]:
        if dim == 2:
            xd_w = np.cross(
                np.hstack(([0, 0], obs[n].angular_velocity)), np.hstack((x - np.array(obs[n].center_position), 0))
            )
            xd_w = xd_w[0:2]
        elif dim == 3:
    ...
    for n in np.arange(N_obs)[ind_obs]:
        ...
        if False:
            ...
            if relative_velocity_norm:  # Nonzero
                ...
                if abs(Gamma_proportional[n]) < 1:
                    ...
                    eigenvalue_magnitude = 1 - 1.0 / abs(Gamma_proportional[n]) ** sticky_surface_power
        ...
        if repulsive_obstacle:
            ...
            if Gamma[n] < (1 + repulsive_gammaMargin):
                ...
                repulsive_speed = ((repulsive_gamma / Gamma[n]) ** repulsive_power - repulsive_gamma) * repulsive_factor
    ...
    if np.sum(ind_nonzero):
        relative_velocity_hat_normalized[:, ind_nonzero] = relative_velocity_hat[:, ind_nonzero] / np.tile(
            relative_velocity_hat_magnitude[ind_nonzero], (dim, 1)
        )
    ...
    if velocity_max is not None:
        ...
        if velocity_norm > velocity_max:
            vel_final = vel_final / velocity_norm * velocity_max
