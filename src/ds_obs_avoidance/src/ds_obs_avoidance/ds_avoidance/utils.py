...
def get_relative_obstacle_velocity(
    position, obstacle_list, E_orth, weights, ind_obstacles=None, gamma_list=None, cut_off_gamma=1e-6
):
...
#     pbd.set_trace()
#     D = compute_diagonal_matrix(Gamma, dim=dim, is_boundary=obs.is_boundary, repulsion_coeff=obs.repulsion_coeff)
...
def get_radius(vec_point2ref, vec_cent2ref=[], a=[], obs=[]):
    ...
    for ii in range(12):  # n_iter
        rotMat = np.array([[np.cos(ang_tot), np.sin(ang_tot)], [-np.sin(ang_tot), np.cos(ang_tot)]])
...
def findRadius(ob, direction, a=[], repetition=6, steps=10):
    ...
    for ii in range(repetition):
        ...
        Gamma = getGammmaValue_ellipsoid(
            ob, np.tile(direction, (steps, 1)).T * np.tile(magnitudeDir, (np.array(ob.x0).shape[0], 1))
        )
...
def compute_R(d, th_r):
    ...
    if d == 2:
        rotMatrix = np.array([[np.cos(th_r), -np.sin(th_r)], [np.sin(th_r), np.cos(th_r)]])
    elif d == 3:
        R_x = np.array([[1, 0, 0,], [0, np.cos(th_r[0]), np.sin(th_r[0])], [0, -np.sin(th_r[0]), np.cos(th_r[0])]])

        R_y = np.array([[np.cos(th_r[1]), 0, -np.sin(th_r[1])], [0, 1, 0], [np.sin(th_r[1]), 0, np.cos(th_r[1])]])

        R_z = np.array([[np.cos(th_r[2]), np.sin(th_r[2]), 0], [-np.sin(th_r[2]), np.cos(th_r[2]), 0], [0, 0, 1]])
...
#                 1
#                 / obs_list[it_obs].sf
#                 * R.T.dot(points - np.tile(np.array([obs_list[it_obs].x0]).T, (1, N_points)))
...
def get_reference_weight(distance, obs_reference_size=None, distance_min=0, distance_max=3, weight_pow=1):
    ...
    weight_ref_displacement = 1 / (dist_temp + 1 - distance_min) - 1 / (distance_max + 1 - distance_min)
