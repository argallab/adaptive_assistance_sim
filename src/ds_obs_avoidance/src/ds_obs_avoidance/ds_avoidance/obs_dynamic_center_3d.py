...
import warnings
from ds_obs_avoidance.ds_avoidance.utils import compute_weights
...
def get_gamma_weight(Gammas, gamma_threshold, Gamma_min=1, pow_gamma=3):
    ...
    if any(ind_nonzero):
        ...
        if any(ind_bellowmin):
            ...
            point_weights[ind_nonzero] = 1 / (Gammas[ind_nonzero] - Gamma_min) - 1 / (gamma_threshold - Gamma_min)
...
def get_dynamic_center_obstacles(
    obs, intersection_lists, gamma_threshold=3, gamma_matr=None, default_kernel_point_weight=0.5
):
    ...
    for it1 in range(n_obs):
        ...
        for it2 in range(n_obs):
            ...
            if Gamma_doubleList[it1][it2] is None:
                Gamma_doubleList[it1][it2] = obs[it2].get_gamma(
                    obs[it1].boundary_points_margin_global, in_global_frame=True
                )
            if Gamma_doubleList[it2][it1] is None:
                Gamma_doubleList[it2][it1] = obs[it1].get_gamma(
                    obs[it2].boundary_points_margin_global, in_global_frame=True
                )

            Gamma_list = np.hstack((Gamma_doubleList[it1][it2], Gamma_doubleList[it2][it1]))

            if any(Gamma_list < gamma_threshold):
                point_weights, object_weights[it2] = get_gamma_weight(Gamma_list, gamma_threshold=gamma_threshold)
                    ...
                    boundary_list = np.hstack(
                        (
                            ...
                            obs[it1].transform_relative2global(
                                obs[it1].transform_global2relative(obs[it2].boundary_points_margin_global)
                                / (np.tile(Gamma_doubleList[it2][it1] ** 2, (dimension, 1)))
                            ),
                        )
                    )

                    kernel_points[:, it2] = np.sum(boundary_list * np.tile(point_weights, (dimension, 1)), axis=1)
        ...
        if any(object_weights):
            ...
            obs[it1].set_reference_point(
                np.sum(kernel_points * np.tile(weights, (dimension, 1)), axis=1), in_global_frame=True
            )


def dynamic_center_3d_old(
    obs, intersection_obs, marg_dynCenter=1.3, N_distStep=3, resol_max=1000, N_resol=16, numbFactor_closest=2
):
    ...
    for it1 in range(N_obs):
        ...
        for it2 in range(it1 + 1, N_obs):
            ...
            if not x_obs1.shape[1] == obs[it2].boundary_points_local.shape[1]:
                ...
                # x_obs1 = x_obs1[0:-1,:]
                x_obs1 = np.tile((x_obs1), (obs[it2].boundary_points_local.shape[1], 1, 1))
            ...
            dist_contact = min_margin_scaling * (obs[it1].get_minimal_distance() + obs[it2].get_minimal_distance())

            ref_dist = max_margin_scaling * (obs[it1].get_maximal_distance() + obs[it2].get_maximal_distance())

            # Inside consideration region -- are obstacles close to each other
            ind = (
                np.sum((x_obs_sf[it2] - np.tile(obs[it1].x0, (x_obs_sf[it2].shape[1], 1)).T) ** 2, axis=0)
                < ref_dist ** 2
            )
