...
class DistanceMatrix:
    ...
    def get_index(self, row, col):
        ...
        return int(int((row - col - 1) + col * ((self._dim - 1) + self._dim - (col)) * 0.5))
...
class Intersection_matrix(DistanceMatrix):
    ...
    def get_bool_triangle_matrix(self):
        raise NotImplementedError("Function was removed. Use 'get_bool_matrix' instead.")
            ...
            if intersection_with_obs1:  # Modify intersecition part
                ...
                if True:
                    ...
                    Gamma_temp = (
                        rotMat[:, :, it_obs2].T.dot(
                            intersection_sf[it_intersect] - np.tile(obs[it_obs2].center_position, (N_inter, 1)).T
                        )
                ...
                if True:
                    ...
                    intersection_sf_temp = np.hstack(
                        (intersection_sf_temp, np.array(obs[it_obs1].x_obs_sf)[:, Gamma < 1])
                    )
                        ...
                        for kk in range(2):
                            ...
                            for ii in range(1, Gamma_steps):
                                ...
                                intersection_sf[it_intersect] = np.hstack(
                                    (intersection_sf[it_intersect], x_obs_sf_interior[:, Gamma < 1])
                                )
                            ...
                            if 1 > obs[it_obs2_].get_gamma(obs[it_obs1_].center_position, in_global_frame=True):
                                intersection_sf[it_intersect] = np.hstack(
                                    [intersection_sf[it_intersect], np.tile(obs[it_obs1_].center_position, (1, 1)).T]
                                )
...
def get_intersections_obstacles(
    obs, hirarchy=True, get_intersection_matrix=False, N_points=30, Gamma_steps=5, representation_type="single_point"
):
    """ 
    OBS_COMMON_SECTION finds common section of two ore more obstacles 
    ...
    for it_obs1 in range(num_obstacles):
        for it_obs2 in range(it_obs1 + 1, num_obstacles):
            ...
            if obs.get_distance(it_obs1, it_obs2) or R_max[it_obs1] + R_max[it_obs2] < LA.norm(
                np.array(obs[it_obs1].center_position) - np.array(obs[it_obs2].center_position)
            ):
                ...
                for kk in range(2):
                    ...
                    if 1 > obs[it_obs2_].get_gamma(obs[it_obs1_].center_position, in_global_frame=True):
                        intersection_points = np.hstack(
                            [intersection_points, np.tile(obs[it_obs1_].center_position, (1, 1)).T]
                        )
    ...
    intersection_cluster_list = get_intersection_cluster(Intersections, obs, representation_type)
...
def get_intersection_cluster(Intersections, obs, representation_type="single_point"):
    ...
    if not obs.index_wall is None:
        ...
        intersecting_obstacles = np.delete(
            intersecting_obstacles, np.nonzero(intersecting_obstacles == obs.index_wall), axis=0
        )
...
def get_single_reference_point(obs, intersection_clusters, Intersections, dim=None, obstacle_weight=None):
    ...
    for ii in range(len(intersection_clusters)):
        ...
        if obs.index_wall in intersection_clusters[ii]:
            ...
            for jj in intersection_clusters[ii]:
                ...
                if Intersections.is_intersecting(jj, obs.index_wall):
            ...
            for jj in intersection_clusters[ii]:
                ...
                if wall_intersection_points.shape[1] == 1:
                    ...
                    dist_intersection = np.linalg.norm(
                        wall_intersection_points - np.tile(obs[jj].position, (wall_intersection_points.shape[1], 1)).T,
                        axis=0,
                    )
                    obs[jj].set_reference_point(
                        wall_intersection_points[:, np.argmin(dist_intersection)], in_global_frame=True
                    )
