...
class IntendedCommandGivenActionAnalysis(object):
    def __init__(self, args):
        ...
        self.file_dir = os.path.join(
            os.path.abspath(os.path.join(os.getcwd(), os.pardir)), "raw_data", self.id + "_p_ui_given_a"
        )
    ...
    def create_p_ui_given_a(self, probabilities):
        ...
        for mode in TRUE_ACTION_TO_COMMAND.keys():
            ...
            for action in TRUE_ACTION_TO_COMMAND[mode].keys():
                ...
                diff_norm_p_ui_given_a[mode][action] = np.linalg.norm(
                    np.array(TRUE_P_UI_GIVEN_A[mode][action].values()) - prob
                )
        ...
        personalized_distributions_dir = os.path.join(
            rospkg.RosPack().get_path("inference_and_correction"), "personalized_distributions"
        )
        pickle.dump(
            p_ui_given_a, open(os.path.join(personalized_distributions_dir, self.id + "_p_ui_given_a.pkl"), "wb")
        )
