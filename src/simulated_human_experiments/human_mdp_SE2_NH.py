...
def visualize_V_and_policy(mdp):
    mdp_params = mdp.get_env_params()
    V = np.array(mdp.get_value_function()).reshape((mdp_params['grid_width'],mdp_params['grid_height'], mdp_params['num_discrete_orientations']))
    ...
    for s in policy_dict.keys():
        grids[s[-1]-1, s[0], s[1]] = policy_dict[s] #first index is the mode. modes are 1 and 2. hence the -1 to account for 0-indexing
...
def simulate_human_SE2_NH_mdp():
    ...
    def get_all_neighbors(cell):
        ...
        all_neighbors = [top_neighbor, bottom_neighbor, left_neighbor, right_neighbor, top_right_neighbor, top_left_neighbor, bottom_left_neighbor, bottom_right_neighbor]
