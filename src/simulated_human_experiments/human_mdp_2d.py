...
def visualize_trajectory(sas_traj, mdp):
    ...
    for t, sas_t in enumerate(sas_traj):
        ...
        plt.arrow(sas_t[0][0], sas_t[0][1], dx, dy, fc="k", ec="k", head_width=0.1, head_length=0.1, length_includes_head=True)
...
def create_mdp_env_param_dict():
    ...
    mdp_env_params['mdp_obstacles'] = create_random_obstacles(  width=mdp_env_params['grid_width'],
                                                                height=mdp_env_params['grid_height'],
                                                                occupancy_measure=OCCUPANCY_LEVEL,
                                                                num_obstacle_patches=np.random.randint(MAX_PATCHES) + 1)
    
    mdp_env_params['mdp_goal_state'] = create_random_goals( width=mdp_env_params['grid_width'],
                                                            height=mdp_env_params['grid_height'],
                                                            num_goals=NUM_GOALS,
                                                            obstacle_list=mdp_env_params['mdp_obstacles'])[0] #make the list a tuple
