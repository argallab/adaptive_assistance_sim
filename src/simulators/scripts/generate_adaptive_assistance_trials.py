...
def generate_experiment_trials(args):
    ...
    for rand_direction_factor, sparsity_level, occupancy_level, num_goals in itertools.product(
        RAND_DIRECTION_LEVELS, SPARSITY_LEVELS, OCCUPANCY_LEVELS, NUM_GOALS
    ):
        ...
        for _ in range(num_reps_per_condition):
            ...
            trial_info_dict["mdp_info_dict"]["shared"][
                "state_value_function"
            ] = collections.OrderedDict()  # one for each goal in the gridworld
            ...
            for _, gw_name in enumerate(grid_world_names):
                ...
                ep["grid_obstacles"] = create_random_obstacles(
                    width=ep["grid_width"],
                ...
                ep["grid_goal_states"] = create_random_goals(
                    width=ep["grid_width"],
                    height=ep["grid_height"],
                    num_goals=num_goals,
                ...
                ep["grid_robot_start_state"] = create_random_start_state(
                    width=ep["grid_width"],
                    height=ep["grid_height"],
                    obstacle_list=ep["grid_obstacles"],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trial_dir",
        dest="trial_dir",
        default=os.path.join(os.getcwd(), "adaptive_trial_dir"),
        help="The directory where trials will be stored are",
    )
    parser.add_argument(
        "--metadata_dir",
        dest="metadata_dir",
        default=os.path.join(os.getcwd(), "adaptive_metadata_dir"),
