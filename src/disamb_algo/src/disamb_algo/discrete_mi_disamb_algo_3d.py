...
class DiscreteMIDisambAlgo3D(object):
    def __init__(self, env_params, subject_id):
        ...
        self.num_sample_trajectories = self.env_params.get("num_sample_trajectories", 600)
    ...
    def get_local_disamb_state(self, prior, current_state, robot_position):
    ...
    def _max_disambiguating_state(self):
        ...
        max_disamb_state = list(max_disamb_state)
        max_disamb_state[-1] = 1
        print(" MODE 1", self.avg_total_reward_for_valid_states[tuple(max_disamb_state)])
        max_disamb_state[-1] = 2
        print(" MODE 2", self.avg_total_reward_for_valid_states[tuple(max_disamb_state)])
        max_disamb_state[-1] = 3
        print(" MODE 3", self.avg_total_reward_for_valid_states[tuple(max_disamb_state)])
        return max_disamb_state
    ...
    def _compute_mi(
        ...
        for i, (vs, vs_continuous) in enumerate(
            ...
            # print(len(p_phm_g_s0[2]), len(p_phm_g_s0[3]), len(p_phm_g_s0[2]) + len(p_phm_g_s0[3]))
            # print("KEYS", p_phm_g_s0.keys())
            ...
            ph_actions_ids = INTERFACE_LEVEL_ACTIONS_TO_NUMBER_ID.values()  # (0,1,2,3)
            ...
            # print(np.sum(kl_list) / self.num_goals)
            self.avg_mi_for_valid_states[vs] = np.mean(kl_list)  # averaged over goals.
